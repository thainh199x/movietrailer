...
const ActionMovieDetail = ({ selectedActionData }) => {
  ...
  useEffect(() => {
    fetchTrailer(selectedActionData.id);
    ...
    // khi không có trailer thì hiển thị poster hoặc backdrop
      ...
      return (
        <img
          ...
          alt="no found"
          src={
            selectedActionData.backdrop_path
              ? `https://image.tmdb.org/t/p/w500${selectedActionData.backdrop_path}`
              : `https://image.tmdb.org/t/p/w500${selectedActionData.poster_path}`
          }
        />
      );
    }
    ...
    <div className={classes.detailTrailer}>
      <div className={classes.detail}>
        ...
        <p className={classes.releaseDate}>
          Release Date: {selectedActionData.release_date}
        </p>
        <p className={classes.vote}>
          Vote: {selectedActionData.vote_average}/10
        </p>
        <p className={classes.overview}>{selectedActionData.overview}</p>
      </div>
      <div>{renderTrailerHandler()}</div>
    </div>
  );
};
export default ActionMovieDetail;
